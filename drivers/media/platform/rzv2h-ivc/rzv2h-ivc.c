...
/**
 * @struct rzv2h_ivc_device: Device structure
 */
struct rzv2h_ivc_device {
	/* Hardware settings */
	struct device *dev;
...
static struct rzv2h_ivc_device *gl_ivc_dev;
...
static const struct rzg2l_cru_callbacks gl_ivc_callbacks_for_cru = {
	.frame_end = rzv2h_ivc_notify_cru_output_done,
};
...
static int rzv2h_ivc_remove(struct platform_device *pdev);
...
{
	uint i;
	struct rzv2h_ivc_device *ivc = gl_ivc_dev;
...
static const struct file_operations rzv2h_ivc_debug_fops = {
	.owner = THIS_MODULE,
	.open = ivc_debug_open,
...
static int ivc_debug_init(struct rzv2h_ivc_device *ivc)
{
	debug_entry_dir = debugfs_create_dir("rzv2h-ivc", NULL);
...

	{
		...
		if (dir) {
			...
			debugfs_create_file("data", 0600, dir, NULL, &rzv2h_ivc_debug_fops);
		}
	}
	{
		uint i;
...
static int ivc_debug_remove(struct rzv2h_ivc_device *ivc)
{
	(void)ivc;
...
static void rzv2h_ivc_write(struct rzv2h_ivc_device *ivc, u32 offset, u32 value)
{
...
int rzv2h_ivc_stream_start(u32 ctx_id, const struct rzv2h_ivc_sensor_mode *mode, int *error, int32_t (*sw_mcfe_external_process_request_v4l2_fr_ss)(uint32_t ctx_id, uint64_t timestamp))
{
	...
	if (is_first) {
		...
		rzv2h_ivc_write(ivc, ISP_FM_CONTEXT, 0x2); /* NFRM_STSEL=0, CONSEL=0b10 Multi Context */
		rzv2h_ivc_write(ivc, ISP_FM_INT_EN, 0x2); /* VVAL_IRPE=1 */
	 ...
	 */
	ret = rzv2h_cru_pm_get(cru);
	if (ret < 0) {
		dev_err(ivc->dev, "rzv2h_cru_pm_get failed with %d\n", ret);
		ret = -EIO;
		goto cleanup;
	}
...
cleanup:
	if (q)
		vb2_queue_release(q);
	rzv2h_ivc_stream_stop(ctx_id);
	return ret;
}
EXPORT_SYMBOL_GPL(rzv2h_ivc_stream_start);
...
int rzv2h_ivc_stream_stop(u32 ctx_id)
{
	...
	if (is_last) {
		 ...
		 */
		rzv2h_ivc_write(ivc, ISP_FM_INT_EN, 0); /* VVAL_IRPE and all=0 */
...
EXPORT_SYMBOL_GPL(rzv2h_ivc_stream_stop);
...
int rzv2h_ivc_notify_isp_config_done(u32 ctx_id)
{
	...
	spin_lock_irqsave(&ivc->state_lock, flags);
	rzv2h_ivc_write(ivc, ISP_FM_MCON, 0x1); /* MCFG_DONE=1 */
	ivc->state.mcfg_done = true;
	if (ivc->state.is_reg_prepared) {
		dev_dbg(ivc->dev, "then START_AXIRX (for IVC buf idx#%d)\n", ivc->state.src->index);
		rzv2h_ivc_write(ivc, ISP_FM_FRCON, 0x1); /* START_AXIRX=1 */
	}
	spin_unlock_irqrestore(&ivc->state_lock, flags);
	return 0;
}
EXPORT_SYMBOL_GPL(rzv2h_ivc_notify_isp_config_done);
...
EXPORT_SYMBOL_GPL(rzv2h_ivc_notify_isp_consumed_frame);
 ...
 * @return	0 on success, <0 on error
 */
static int rzv2h_ivc_frame_start(struct received_src_slot *slot)
{
	int result;
	struct rzv2h_ivc_device *ivc = gl_ivc_dev;
	...
	if (result == 0) {
...
#endif
		rzv2h_ivc_write(ivc, ISP_AXIRX_PLNUM, cru->exposure == 2 ? 1 : 0); // PNUM
		...
		if (cru->exposure == 2)	{
			if (cru->sensor_settings.is_hdr) {
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDH_P1, (addr + cru->format.sizeimage) >> 32);
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDL_P1, (addr + cru->format.sizeimage) & 0xffffffff);
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDH_P0, addr >> 32);
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDL_P0, addr & 0xffffffff);
			} else {
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDH_P0, (addr + cru->format.sizeimage) >> 32);
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDL_P0, (addr + cru->format.sizeimage) & 0xffffffff);
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDH_P1, addr >> 32);
				rzv2h_ivc_write(ivc, ISP_AXIRX_SADDL_P1, addr & 0xffffffff);
			}
		} else {
			rzv2h_ivc_write(ivc, ISP_AXIRX_SADDH_P0, addr >> 32);
			rzv2h_ivc_write(ivc, ISP_AXIRX_SADDL_P0, addr & 0xffffffff);
		}
		rzv2h_ivc_write(ivc, ISP_AXIRX_HSIZE, cru->format.width & 0x1fff);
		rzv2h_ivc_write(ivc, ISP_AXIRX_VSIZE, cru->format.height & 0x1fff);
#ifdef CONFIG_VIDEO_RZG2L_CRU_IMAGE_PROCESSOR
		rzv2h_ivc_write(ivc, ISP_AXIRX_PXFMT, context->dtype | 0x00010000); /* CLFMT=1, DTYPE=value in rzg2l_cru_formats[] */
#else
		rzv2h_ivc_write(ivc, ISP_AXIRX_PXFMT, context->dtype/* | 0x00010000*/); /* CLFMT=1, DTYPE=value in rzg2l_cru_formats[] */
#endif
		rzv2h_ivc_write(ivc, ISP_AXIRX_STRD, context->strd);
		spin_lock_irqsave(&ivc->state_lock, flags);
 ...
 */
static irqreturn_t rzv2h_ivc_irq(int irq, void *data)
{
	struct rzv2h_ivc_device *ivc = data;
...
static int rzv2h_ivc_probe(struct platform_device *pdev)
{
	...
	dev_info(priv->dev, "RZ/V2H Input Video Control Driver %pR, ncru = %d\n", res, ncru);
	/* List all CRUs */
	for (i = 0; i < ncru; ++i) {
	...
	ret = devm_request_irq(priv->dev, irq, rzv2h_ivc_irq, IRQF_SHARED,
			       KBUILD_MODNAME, priv);
	if (ret) {
		dev_err(priv->dev, "failed to set up irq\n");
		return ret;
...
static int rzv2h_ivc_remove(struct platform_device *pdev)
{
	struct rzv2h_ivc_device *ivc = platform_get_drvdata(pdev);

	/* Stop IRQ */
	rzv2h_ivc_write(ivc, ISP_FM_INT_EN, 0);
...
static struct platform_driver rzv2h_ivc_platform_driver = {
...
module_platform_driver(rzv2h_ivc_platform_driver);

MODULE_ALIAS("rzv2h-ivc");
MODULE_AUTHOR("Maki Hirai <maki.hirai.wt@bp.renesas.com>");
MODULE_DESCRIPTION("Renesas RZ/V2H Input Video Control Driver");
MODULE_LICENSE("GPL");
