...
#define v4l2_subdev_lock_and_get_active_state(...) 0
#define v4l2_subdev_get_locked_active_state(...) 0
#define v4l2_subdev_unlock_state(...) ((void)0)
struct v4l2_subdev_state {};

#define IMX415_PIXEL_ARRAY_TOP	  0
#define IMX415_PIXEL_ARRAY_LEFT	  0
#define IMX415_PIXEL_ARRAY_WIDTH  3840
#define IMX415_PIXEL_ARRAY_HEIGHT 2160
...
/* all-pixel 4-lane 720 Mbps 15.74 Hz mode */
static const struct imx415_reg imx415_mode_4_720[] = {
	{ IMX415_VMAX, 0x08CA },
	{ IMX415_HMAX, 0x07F0 },
	{ IMX415_LANEMODE, IMX415_LANEMODE_4 }, /* CHECKME: was LANEMODE_2 */
...
static int imx415_write(struct imx415 *sensor, u32 addr, u32 value)
{
	...
	{
		char buff[20];
		int len = (addr >> IMX415_REG_SIZE_SHIFT) & 3;
		char *p = buff;
		unsigned int i;

		for (i = 0; i < len; ++i)
			p += snprintf(p, sizeof(buff) - (p - buff), " 0x%02x", data[i]);
		dev_dbg(sensor->dev, "%s 0x%04x %s\n", __func__,
			addr & IMX415_REG_ADDR_MASK, buff);
	}
...
static int imx415_stream_on(struct imx415 *sensor)
{
	...
	if (!sensor->slave_mode)
		ret = imx415_write(sensor, IMX415_XMSTA, IMX415_XMSTA_START);
	return ret;
}
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	fse->min_width = fse->max_width / 2;
	fse->max_height = IMX415_PIXEL_ARRAY_HEIGHT;
	fse->min_height = fse->max_height / 2;
	return 0;
}
...
//		fmt->format.width = IMX415_PIXEL_ARRAY_WIDTH /2 ;
//		fmt->format.height = IMX415_PIXEL_ARRAY_HEIGHT / 2;
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	format->code = MEDIA_BUS_FMT_SBGGR12_1X12;
...
static int imx415_identify_model(struct imx415 *sensor)
{
	...
	case IMX415_CHIP_ID:
		...
		break;
	case IMX715_CHIP_ID:
		dev_info(sensor->dev, "Detected IMX715 image sensor\n");
		break;
	default:
		dev_info(sensor->dev, "Detected image sensor\n");
		break;
	}
...
static int imx415_parse_hw_config(struct imx415 *sensor)
{
	...
	if (device_property_read_bool(sensor->dev, "slave-mode"))
		sensor->slave_mode = true;
	...
	for (i = 0; i < bus_cfg.nr_of_link_frequencies; ++i) {
		...
		for (j = 0; j < ARRAY_SIZE(supported_modes); ++j) {
			if (sensor->num_data_lanes != supported_modes[j].lanes)
				continue;
			dev_err(sensor->dev, "[%d]: %lld %lld\n", j, bus_cfg.link_frequencies[i] * 2,  supported_modes[j].lane_rate);
...
static int imx415_probe(struct i2c_client *client)
{
	...
	sensor->format.width = IMX415_PIXEL_ARRAY_WIDTH;
	sensor->format.height = IMX415_PIXEL_ARRAY_HEIGHT;
